...
def style_table(df_display):
    ...
    # 3. Estilo para la Fila de Promedios (índice -1)
    def highlight_average_row(row):
        is_average = (row.name == df_display.index[-1])
        if is_average:
            return ['font-weight: bold; background-color: #E6F3FF;'] * len(row) # Azul muy claro
        return [''] * len(row)

    styler = styler.apply(highlight_average_row, axis=1)
...
try:
    ...
    if not df_tasas_mora.empty:
        ...
        # --- CALCULAR Y AÑADIR LA FILA DE PROMEDIOS ---
        
        # 1. Crear una Series para los promedios
        avg_row = pd.Series(index=df_display.columns)
        
        # 2. Calcular el promedio para las columnas numéricas
        
        # Saldo Capital Total (Monto)
        # Calculamos el promedio del saldo ANTES de formatearlo (usando df_tasas_mora.iloc)
        avg_saldo = df_tasas_mora.iloc[:, 1].mean()
        avg_row.iloc[1] = format_currency(avg_saldo) # Formateo de moneda
        
        # Tasas de Mora
        for i, col in enumerate(tasa_cols):
            # Obtener los valores numéricos de las tasas (sin strings vacíos)
            # Usamos df_tasas_mora para los valores *originales* flotantes
            rate_values = df_tasas_mora.iloc[:, i + 2] # Las tasas comienzan en el índice 2
            
            # Solo promediar valores que NO son 0 (o sea, donde sí hay datos reportados)
            # Para esto, necesitamos el DataFrame original sin el corte de fechas para el cálculo,
            # pero dado que no lo tenemos aquí, promediamos todos los floats y confiamos en que 
            # los 0.00% son tasas reales y no NA.
            
            avg_rate = rate_values.mean()
            avg_row.iloc[i + 2] = format_percent(avg_rate) # Formateo de porcentaje
        
        # Columna de Mes de Apertura (Etiqueta)
        avg_row.iloc[0] = 'PROMEDIO'
        
        # 3. Añadir la fila al DataFrame de visualización
        df_display.loc['avg_row'] = avg_row
        
        
        # 4. APLICAR ESTILOS CON STYLER (Heatmap y Encabezados)
